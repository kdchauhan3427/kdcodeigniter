...
class Admin extends CI_Controller
{
    ...
    function product_bundle($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            $ins = $this->db->insert('product', $data);
            if(!$ins){
   ...
   /* Manage Frontend Captcha Settings Credentials */
    ...
    //============================by kuldeep========================
    function cashback_achiever($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('user')) {
            redirect(base_url() . 'admin');
        }
        ...
        if ($para1 == 'delete') {
            ...
            $page_data['all_wallet_loads'] = $this->db->get('bonus_transaction')->result_array();
            $page_data['achiever'] = $this->db->get_where('user',['cashback !='=>0])->result_array();
            // echo $json_encode($page_data);die;
            $this->load->view('back/admin/cashback_achiever_list', $page_data);
        }   else if ($para1 == 'user_view') {
            $page_data['user_data'] = $this->db->get_where('user', array(
                'user_id' => $para2
            ))->result_array();
            ...
            $page_data['page_name'] = "cashback_achiever";
            $page_data['all_bonus_transactions'] = $this->db->get('wallet_load')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    /* bonus_transaction Management */
    function bonus_transaction($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('user')) {
            redirect(base_url() . 'admin');
        }
        ...
        if ($para1 == 'delete') {
            ...
            $this->db->order_by('id', 'desc');
            $page_data['all_wallet_loads'] = $this->db->get('bonus_transaction')->result_array();
            $this->load->view('back/admin/bonus_transaction_list', $page_data);
        }   else if ($para1 == 'user_view') {
            $page_data['user_data'] = $this->db->get_where('user', array(
                'user_id' => $para2
            ))->result_array();
            ...
            $page_data['page_name'] = "bonus_transaction";
            $page_data['all_bonus_transactions'] = $this->db->get('wallet_load')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    function direct_income($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('user')) {
            redirect(base_url() . 'admin');
        }
        ...
        if ($para1 == 'delete') {
            ...
            $this->db->order_by('id', 'desc');
            $page_data['all_wallet_loads'] = $this->db->get('direct_income')->result_array();
            $this->load->view('back/admin/direct_income_list', $page_data);
        }   else if ($para1 == 'user_view') {
            $page_data['user_data'] = $this->db->get_where('user', array(
                'user_id' => $para2
            ))->result_array();
            ...
            $page_data['page_name'] = "direct_income";
            $page_data['all_direct_incomes'] = $this->db->get('direct_income')->result_array();
            // echo '<pre>';print_r($page_data);die;
            $this->load->view('back/index', $page_data);
        }
    }
    function app_banner($para1 = '', $para2 = '', $para3 = '')
    {
        $page_data['page_name'] = "banner";
        $this->db->order_by('id', 'desc');
        $page_data['all_slider'] = $this->db->get('app_banners')->result_array();
        $this->load->view('back/index', $page_data);
    }
    function edit_app_banner($id){
        if($this->input->method()=='post'){
            $app_banners['banner_title'] = $this->input->post('banner_title');
            if(array_key_exists('banner_subtitle', $this->input->post())){
                $app_banners['banner_subtitle'] = $this->input->post('banner_subtitle');    
            }
            if(!empty($_FILES['banner_img']['name'])){
                $app_banners['banner_img'] = 'app_banners_'.rand(0000,9999).time().'.jpg';
                move_uploaded_file($_FILES['banner_img']['tmp_name'],'uploads/app_banners/'.$app_banners['banner_img']);
            }
            // echo $id;die;
            // echo "<pre>";print_r($app_banners);die;
            $this->db->where('id',$id)->update('app_banners',$app_banners);
            // echo $this->db->last_query();die;
            redirect(base_url().'admin/app_banner/');

        }else{
           $page_data['page_name'] = "banner_edit";
            $page_data['banner_data'] = $this->db->get_where('app_banners',['id'=>$id])->row();
            $this->load->view('back/index', $page_data); 
        }
    }
    function add_app_banner(){
        if($this->input->method()=='post'){
            ...
            $app_banners['banner_img'] = 'app_banners_'.rand(0000,9999).time().'.jpg';
            $this->db->insert('app_banners',$app_banners);
            move_uploaded_file($_FILES['banner_img']['tmp_name'],'uploads/app_banners/'.$app_banners['banner_img']);
    ...
    function order_statuses($para1 = '', $para2 = '')
    {
        ...
        elseif ($para1 == 'list') {
            ...
            $this->load->view('back/admin/order_statuses_list', $page_data);
        } else {
            $page_data['page_name']      = "order_statuses";
            $page_data['all_sales'] = $this->db->get('sale')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    function userIncome($para1 = '', $para2 = '', $para3 = '')
    {
        $page_data['page_name'] = "userIncome";
        $this->db->order_by('id', 'asc');
        $page_data['all_income'] = $this->db->get('user_income')->result_array();
        $this->load->view('back/index', $page_data);
    }
    function edit_income($id){
        if($this->input->method()=='post'){
            // $data = $this->db->get_where('user_income',['id'=>$id])->row();
            $input = $this->input->post();
            $this->db->where('id',$id)->update('user_income',$input);
            // echo '<pre>';print_r($input);die;
            redirect(base_url().'admin/userIncome/');

        }else{
           $page_data['page_name'] = "edit_income";
            $page_data['income_data'] = $this->db->get_where('user_income',['id'=>$id])->row();
            // echo '<pre>';print_r($pagedata);die;
            $this->load->view('back/index', $page_data); 
        }
    }
    function sendnotifications($para1 = '', $para2 = '', $para3 = '')
    {
        if($this->input->method()=='post'){
            // $data = $this->db->get_where('user_income',['id'=>$id])->row();
            $input = $this->input->post();
            $input['user'] = ($input['user']!='')?$input['user']:'0';
            // echo '<pre>';print_r($input);die;
            // $this->db->where('id',$id)->update('user_income',$input);
            $sd = $this->sendmultiplenotification($input['title'],$input['message'],$input['notification_type'],$input['user']);
            // echo $sd;die;
            redirect(base_url().'admin/sendnotifications/');

        }else{
            $page_data['page_name'] = "sendnotifications";
            $this->db->order_by('id', 'asc');
            $page_data['all_income'] = $this->db->get('user_income')->result_array();
            $this->load->view('back/index', $page_data); 
        }
    ...
    function sendmultiplenotification($title,$message,$type,$user){
        require_once(APPPATH . 'libraries/sendnotification/firebase.php');
        require_once(APPPATH . 'libraries/sendnotification/push.php');
        ...
        if($user!='0'){
            ...
            $regarray = array();
            if($type=='multiple'){
                $findappids = $this->db->get('androidappid')->result();
                if(!empty((array)$findappids)){
                ...
                // echo 'dfads';die;
                $findappids = $this->db->get_where('androidappid',['user_id'=>$user])->row();
                if(!empty($findappids)){
                       $regarray[] = $findappids->appkey;
                }
            }
            echo '<pre>';print_r($regarray);die;
    ...
    function check_sale(){
        ...
        foreach ($data as $row) {
           $days = $this->crud_model->get_date_difference($row->created_at);
            if($days<=30){
                $sales = $this->db->get_where('sale',array('buyer'=>$row->user_id,'bonus_given'=>0))->result();
                
                if($row->user_id==38){
                    $salestotal = 0;
                    foreach ($sales as $key => $sale) {
                        if(json_decode($sale->payment_status)[0]->status=='paid'){
                            $salestotal += $sale->grand_total;
                        }
                    }
                    if($salestotal>1000 && $salestotal <=5000){
                        foreach ($sales as $key => $sale) {
                            if(json_decode($sale->payment_status)[0]->status=='paid'){
                                $transaction['sale_id'] = $sale->sale_id;
                                $transaction['sale_code'] = $sale->sale_code;
                                $transaction['buyer_id'] = $sale->buyer;
                                $transaction['amount'] = floor($sale->grand_total);
                                $did = $this->crud_model->refer_purchase_amount($transaction,$sale->buyer,$count=0);
                                // $this->db->insert('bonus_transaction',$transaction);
                                $this->db->where('sale_id',$sale->sale_id)->update('sale',['bonus_given'=>1]);
                                echo 'Done';die;
                            }
                        } 
                    }
                    // $salesum = floor($this->db->select_sum('grand_total')->where(['buyer'=>38,'bonus_given'=>0])->get('sale')->row()->grand_total);
                    echo $salestotal.'<br>';
                    // foreach ($sales as $key => $sale) {

                    // }
                    echo '<pre>';print_r(json_decode($sales[0]->payment_status)[0]->status);die;
                }
            } 
        }
        // echo '<pre>';print_r($this->session->userdata());die;
    }
    function provide_coupon_cashback(){
        // echo 'dsfdsfad';die;
        $data = $this->db->get_where('user',['cashback'=>0])->result();
        // echo json_encode($data);die;        
        foreach ($data as $row) {
            if($row->cashback==0){
                $sales = $this->db->get_where('sale',array('buyer'=>$row->user_id))->result();
                $salestotal = 0;
                foreach ($sales as $key => $sale) {
                    if(json_decode($sale->payment_status)[0]->status=='paid'){
                        $salestotal += $sale->grand_total;
                ...
                if($salestotal>=5000){
                    $wallet_amt = base64_encode(base64_decode($row->wallet)+5000);
                    $d['total_shopping_amount'] = $salestotal;
                    $d['cashback_count'] = 1;
                    $d['cashback_date'] = time();
                    $this->db->where('user_id',$row->user_id)->update('user',['cashback_data'=>json_encode($d),'wallet'=>$wallet_amt,'cashback'=>1]);
                }
            }
    ...
    function direct_incomes(){
        $data = $this->db->get('user')->result(); 
        foreach ($data as $row) {
           $days = $this->crud_model->get_date_difference($row->created_at);
            if($days<=30){
                if($row->user_id==15){
                    $referusers = $this->db->select('user_id')->get_where('user',array('referid'=>$row->user_id))->result();
                    $refercount = count($referusers);
                    $sale_count = $this->crud_model->refer_from_sale($row->user_id,$referusers);
                    $saledata = $this->db->get_where('direct_income',array('user_id'=>$row->user_id))->row();
                    if(empty($saledata)){
                        $this->db->insert('direct_income',$sale_count);
                        echo "Insertion Done";die;
                    }else{
                        $this->db->where('id',$saledata->id)->update('direct_income',$sale_count);
                        echo "updated Successfuly";die;
                    }
