...
class APIController extends CI_Controller
{
    ...
    function product_details(){
        header('Content-Type: application/json');
            ...
            if(!empty($products)){
                $json['status'] = 1;
                $json['product_id'] = $products->product_id;
                $json['description'] = trim($products->description,' ');
                $json['wished'] = (in_array($products->product_id, $ids))?true:false;
                $json['category_name'] = $this->crud_model->get_type_name_by_id('category',$products->category,'category_name');
                $json['title'] = $products->title;
                $json['sale_price'] = round($products->sale_price);
                ...
                if(!empty($ps)){
                    foreach($ps as $row1){
                        $json['additional_fields'][$row1['name']] = $row1['value'];
                    }
                }
    ...
    function myOrders(){
        ...
        // echo '<pre>';print_r(count((array)$sale_data));die;
        if(!empty($sale_data)){
            $i=0;
            foreach ($sale_data as $key => $value) {
                if($value->sale_datetime==null){
                    continue;
                }
                $c = array();
                $product_details = json_decode($value->product_details,true);
                $json[$i]['orderid'] = $value->sale_code;
                $json[$i]['totalprice'] = $value->grand_total;
                $shipping = json_decode($value->shipping_address);
                // $json[$i]['shipping_address'] = $shipping->full_name.', '.$shipping->house_no.', '.$shipping->street.', '.$shipping->landmark.', '.$shipping->city.', '.$shipping->state.' - '.$shipping->pin_code;
                $json[$i]['placed_date'] = date('d M, Y',$value->sale_datetime);
    ...
    function ordersdetail(){
        ...
        $sale_data = $this->db->get_where('sale',['buyer'=>$user->user_id,'sale_code'=>$orderid])->row();
        // echo '<pre>';print_r(count((array)$sale_data));die;
        if(!empty($sale_data)){
            $i=0;
                $c = array();
                $product_details = json_decode($sale_data->product_details,true);
                $json['orderid'] = $sale_data->sale_code;
                $json['totalprice'] = $sale_data->grand_total;
                $shipping = json_decode($sale_data->shipping_address);
                ...
                $json['placed_date'] = date('d M, Y',$sale_data->sale_datetime);
                // echo '<pre>';print_r($sale_data);die;
                $j=0;
                foreach ($product_details as $key1 => $value1) {
                    $ids = json_decode($this->db->get_where('user',array('user_id'=>$user->user_id))->row()->wishlist,true);
                    $value1['wished'] = (in_array($value1->product_id, $ids))?true:false;
                    $shipping = json_decode($sale_data->shipping_address);
                    // $value1['shipping_phone'] = $shipping->mobile_number;
                    // $value1['alternate_mobile'] = ($shipping->alternate_mobile!='')?$shipping->alternate_mobile:'';
                    // $value1['payment_type'] = str_replace('_', ' ', ucwords($sale_data->payment_type));
                    // $value1['purchase_date'] = date('d M, Y',$sale_data->sale_datetime);
                    // $value1['payment_status'] = json_decode($sale_data->payment_status)[0]->status;
                    // $value1['order_status'] = ($sale_data->order_status==0)?'pending':(($sale_data->order_status==1)?'Delivered':(($sale_data->order_status==2)?'On Delivery':'Cancelled'));
                    $value1['image'] = explode(',',$value1['image'])[0];
                    unset($value1['category_name'],$value1['shipping'],$value1['discount'],$value1['coupon']);
                    $c[] = $value1;
                    $j++;
                ...
                $json['itemqty'] = $j;
                $json['items'] = $c;
                $payment_details = json_decode($sale_data->payment_details);
                $json['total_amount'] = $payment_details->total_amount;
                $json['total_shipping'] = $payment_details->total_shipping;
                $json['total_discount'] = $payment_details->total_discount;
                $json['grand_total'] = $payment_details->grand_total;
    ...
    function cancelOrder(){
        ...
        $val = $this->db->get_where('sale',['buyer'=>$user->user_id,'sale_code'=>$orderid])->row();
        // echo json_encode($val);die;
        if(!empty($val)){
            $d['order_status'] = 3;
            $s = json_decode($val->delivery_status,true);
            $s[0]['status'] = 'cancelled';
            $s[0]['comment'] = $reason;
            $d['delivery_status'] = json_encode($s);
            // $s[0]['delivery_time'] = date('d M,Y',$s[0]['delivery_time']);
            $this->db->where('sale_id',$val->sale_id)->update('sale',$d);
            $json['status'] = 1;
            $json['msg'] = 'Order cancelled';
        }else{
            $json['status'] = 0;
            $json['msg'] = 'Order Not available';
        }
        echo json_encode($json);die;
    ...
    function reorder(){
        ...
        $val = $this->db->get_where('sale',['buyer'=>$user->user_id,'sale_code'=>$orderid])->row();
        if(!empty($val)){
            $products_data = json_decode($val->product_details);
